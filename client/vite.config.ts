...
/* Relative import on purpose: vite's config loader bundles relative `.ts` files via esbuild,
 * but resolves package imports through Node ESM, which won't read the `.ts` files in our
 * workspace-linked `@aether/shared` package. Application code should still use `@aether/shared/...`. */
import { DEFAULT_SERVER_FPS } from "../shared/net/constants";
...
export default ({ mode, command }) => {
  ...
  console.log(`🛠 METERED_APP_NAME: ${process.env.METERED_APP_NAME ? "(set)" : "(empty)"}`);
  ...
  return defineConfig({
    ...
    define: {
      ...
      "process.env.METERED_APP_NAME": JSON.stringify(process.env.METERED_APP_NAME ?? ""),
      "process.env.METERED_API_KEY": JSON.stringify(process.env.METERED_API_KEY ?? ""),
      "process.env.METERED_REGION": JSON.stringify(process.env.METERED_REGION ?? ""),
    },
    server: {
      host: true,
    },
